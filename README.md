...
* `nextMoveRank`, `nextMovePrior`, `nextMoveHumanPrior`: The order, prior, and humanPrior of `nextMove` in `moveInfos` if exists.
...
* `unsettledness`: The situation tend to be 'exciting' if this is greater than 20. It is defined as the sum of (1 - |ownership|) for all stones on the board. (It is indicated by red dots in the score chart in [LizGoban](https://github.com/kaorahi/lizgoban). [ref](https://github.com/sanderland/katrain/issues/215)) This field is also added into each element in `moveInfos` if `includeOwnership` is true in the query. (Tip: Use `jq -c '.moveInfos |= map(del(.board, .ownership))'` to delete `board` and `ownership` in `moveInfos`.)
* `blackUnsettledness`, `whiteUnsettledness`, `territoryUnsettledness`: The sum of (1 - |ownership|) on black stones, white stones, and empty grids, respectively. They are also added into `moveInfos` like `unsettledness`.
* `board`: 2D array of "X", "O", or "." for the current board. This field is also added into each element in `moveInfos` if `includeOwnership` is true in the query. (Tip: Use `jq -c 'del(.moveInfos[].board)'` to delete them.)
...
* All fields in `rootInfo`, `query`, and `overrideSettings` are also copied directly under the response. This enables easy access in pandas and jq (`{turnNumber, winrate}` instead of `{turnNumber, winrate: .rootInfo.winrate}`).
...
* If exists, the following fields are copied into `rootInfo`: `blackUnsettledness`, `whiteUnsettledness`, `territoryUnsettledness`, `unsettledness`, `blackMoyo`, `whiteMoyo`, `moyoLead`. They also appear in `nextRootInfo`.
...
* -scan-humansl-ranks: Loop through `rank_9d` to `rank_20k` on `humanSLProfile` for each query.
