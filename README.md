...
* `blackUnsettledness`, `whiteUnsettledness`, `territoryUnsettledness`: The sum of (1 - |ownership|) on black stones, white stones, and empty grids, respectively. They are also added into each element in `moveInfos` if available.
* `board`: 2D array of "X", "O", or "." for the current board. The field `board` is also added into each element in `moveInfos`. (Tip: Use `jq -c 'del(.moveInfos[].board)'` to delete them.)
