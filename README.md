...
* `blackUnsettledness`, `whiteUnsettledness`, `territoryUnsettledness`: The sum of (1 - |ownership|) on black stones, white stones, and empty grids, respectively. These fields are also added into each element in `moveInfos` if `includeOwnership` is true in the query. (Tip: Use `jq -c '.moveInfos |= map(del(.board, .ownership))'` to delete `board` and `ownership` in `moveInfos`.)
